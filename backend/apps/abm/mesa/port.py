...
class Port(Agent):
    ...
    port_file_path = os.path.join(current_dir, 'filtered_ports_with_x_y.csv')

    with open(port_file_path, 'r') as port_data:
    ...
    def __init__(self, unique_id, model, port_data, policy=None):
        ...
        if policy is not None:
            ...
            if hasattr(model, 'port_policy') and model.port_policy and len(model.port_policy) > 0:
        ...
        self.revenue = 0
        self.scrubber_revenue = 0
        self.non_scrubber_revenue = 0
        
        # base fees for different ship types
        self.base_fees = {
            "cargo": 100,
            "tanker": 120,
            "fishing": 50,
    ...
    def calculate_docking_fee(self, ship):
        ...
        The fee is adjusted dynamically: the more full the port, the higher the fee 
        """
    ...
    def dock_ship(self, ship):
        ...
        if self.current_capacity < self.port_capacity:
            ...
            # Track separate revenue by ship type
            if ship.is_scrubber:
                self.scrubber_revenue += fee
                revenue_type = "scrubber"
            else:
                self.non_scrubber_revenue += fee
                revenue_type = "non-scrubber"
                
            self.current_capacity += 1
            self.docked_ships.append(ship)
            print(f"Port {self.name}: {revenue_type.title()} ship {ship.unique_id} docked, fee charged: {fee:.2f}, total revenue: {self.revenue:.2f}")
            return True
        return False
