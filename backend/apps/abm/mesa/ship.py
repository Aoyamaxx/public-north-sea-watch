...
class Ship(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        avg_penalty = self.model.get_average_penalty() if hasattr(self.model, 'get_average_penalty') else 0
    ...
    def step(self):
        ...
        if self.exiting:
            ...
            if self.pos == target_pos:
                ...
                # Check if the model has remaining ships to spawn or should generate a new one
                if hasattr(self.model, 'remaining_ships') and self.model.remaining_ships > 0:
                    # Use existing ship spawning logic if there are ships waiting to be spawned
                    self.model.spawn_ship(self.model.next_ship_id)
                    self.model.next_ship_id += 1
                    self.model.remaining_ships -= 1
                else:
                    # Generate a new ID that won't conflict with existing ships
                    # Use a high base ID to avoid conflicts with initial ships
                    new_id = 10000 + (self.model.next_ship_id % 1000)
                    self.model.next_ship_id = new_id + 1
                    # Spawn a replacement ship with the new ID
                    self.model.spawn_ship(new_id)
                return   
            ...
            if self.route and self.current_target_index < len(self.route):
                ...
                if self.pos == target_pos or target_pos in self.model.grid.get_neighborhood(self.pos, moore=True, include_center=True):
                    ...
                    if success:
                        ...
                        if hasattr(target_port, 'scrubber_policy') and target_port.scrubber_policy == "tax" and self.is_scrubber:
                            self.penalty += 0.5
                            if hasattr(self.model, 'scrubber_penalty_sum'):
                        ...
                        if self.is_scrubber and hasattr(target_port, 'allow_scrubber') and not target_port.allow_scrubber:
                            self.penalty += 1
                            if hasattr(self.model, 'scrubber_penalty_sum'):
